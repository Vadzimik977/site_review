...
### 11. 🟡 UX: Модалка отличается от визуала всего сайта

**Приоритет:** 🟡 Средний

**Описание:**
Модальное окно (при просмотре ячейки/аренды) визуально сильно отличается от остального сайта. Как будто это отдельное приложение.
...
**Рекомендации:**
1. Унифицировать дизайн модалки с основным сайтом
2. Сохранить хедер МегаСклада
3. Использовать единую цветовую схему и типографику

---

### 12. 🔴 UX: Кнопка "Оформить аренду" не работает
...
### 13. 🟡 UX: Выбор периода — неинтуитивный дизайн
...
### 14. 🔴 БАГ: Оплата по СБП не работает
...
### 15. 🔴 БАГ: 500 ошибка при создании подписки / загрузке подписок

**Приоритет:** 🔴 Критичный

**Описание:**
При попытке оплаты или загрузке списка подписок в ЛК возникает 500 ошибка сервера. Подписка не создаётся.

**Логи ошибки:**
```
[Error] Failed to load resource: the server responded with a status of 500 (Internal Server Error) (subscriptions, line 0)
[Error] Payment creation failed: – Object
Object
    (анонимная функция) (RentModal.vue:267)
```

**Возможные причины:**
1. Бэкенд не может создать подписку (ошибка в БД или интеграции с ЮKassa)
2. Невалидные данные, переданные с фронтенда
3. Ошибка в API `/subscriptions` — сервер падает с 500

**Рекомендации:**
1. Проверить логи бэкенда (API `jokingly-animating-ouzel.cloudpub.ru`)
2. Добавить обработку ошибки на фронтенде — показать пользователю понятное сообщение вместо молчаливого падения
3. Добавить retry-логику или fallback
...
| 10 | Модалка отличается от визуала сайта | 🟡 |
| 11 | Кнопка "Оформить аренду" не работает | 🔴 |
| 12 | Выбор периода — неинтуитивный дизайн | 🟡 |
| 13 | Оплата по СБП не работает | 🔴 Критичный |
| 14 | 500 ошибка при создании подписки | 🔴 Критичный |

---

## Приоритеты для доработки

1. 🔴 **Критичный:** Исправить 500 ошибку при создании подписки (проверить бэкенд)
2. 🔴 **Критичный:** Исправить оплату по СБП (ЮKassa рекуррентные платежи)
3. 🔴 **Высокий:** Исправить баг с формой калькулятора (кривое отображение)
4. 🔴 **Высокий:** Добавить ссылку на политику конфиденциальности в чекбокс согласия
5. 🔴 **Высокий:** Исправить валидацию — показывать ошибку под конкретным полем
...
9. 🟡 **Средний:** Унифицировать дизайн модалки с основным сайтом
...
### Баг #11: Модалка отличается от визуала сайта
![Модалка отличается от сайта](modal_visual_mismatch.png)
- **Описание:** Модальное окно визуально сильно отличается от основного сайта
